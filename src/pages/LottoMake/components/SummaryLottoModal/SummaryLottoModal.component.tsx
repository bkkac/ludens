...
const SummaryLottoModal: SFC<ISummaryLottoModalProps & DefaultProps> = ({
  ...
  const RenderLottoList = () => {
    const groupingLottoListObject: { [name in TLottoType]?: (ILottoNumber & { seq?: number })[] }
    ...
    const GroupingLottoListComponent = keys(groupingLottoListObject).map((lottos, lottosIndex) => {
      const LottoListComponent = groupingLottoListObject[lottos as TLottoType]?.map((lotto, lottoIndex) => {
        return (
      ...
      return (
        <div className="row mt-4" key={`lotto-type-${lottosIndex}`}>
          <div className="col">
            <div className="row">
              <div className="col lotto-name-type">{lottoTypeNames[lottos as TLottoType]}</div>
            </div>
