...
for i in "${!array[@]}"; do
  ...
  ssh ubuntu@${array[i]} "cd  /var/www/luden && sudo git checkout staging && sudo git pull \"https://cheddotechnology:cheddotechnology123@gitlab.com/privatedevelopment/luden.git\" && sudo npm i && sudo npm run build && sudo rm -R ../html/* && sudo mv build/* ../html/ && sudo systemctl restart nginx"
done
echo "Finish deploy"
